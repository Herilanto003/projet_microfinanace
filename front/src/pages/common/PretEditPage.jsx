...
// Schema de validation avec Zod (identique à PretAddPage)
...
export default function PretEditPage() {
  const navigate = useNavigate();
  const { id } = useParams();
  const [updatePret, { isLoading: isUpdating }] = useUpdatePretMutation();

  // Récupération des données du prêt
  const {
    data: pretData,
    isLoading: isLoadingPret,
    error: pretError,
  } = useGetPretByIdQuery(id);
  ...
  // Effet pour remplir le formulaire quand les données du prêt sont chargées
  useEffect(() => {
    if (pretData?.data) {
      const pret = pretData.data;
      reset({
        titre_pret: pret.titre_pret,
        description_pret: pret.description_pret,
        statut_pret: pret.statut_pret,
        date_pret: pret.date_pret,
        montant_pret: pret.montant_pret,
        client_id: pret.client_id.toString(),
        periode: pret.periode.toString(),
        taux_pret: pret.taux_pret,
        approbation_pret: pret.approbation_pret,
      });
    }
  }, [pretData, reset]);
  ...
  useEffect(() => {
    if (pretError) {
      ...
      console.error("Erreur prêt:", pretError);
    }
    if (clientsError) {
      toast.error("Erreur lors du chargement des clients");
      console.error("Erreur clients:", clientsError);
    }
  }, [pretError, clientsError]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await updatePret({ id, ...pretData }).unwrap();
      toast.success("Prêt modifié avec succès !");
      navigate("/compte/admin/prets");
    } catch (error) {
      console.error("Erreur lors de la modification:", error);
      toast.error("Erreur lors de la modification du prêt");
    }
  };
  ...
  if (pretError) {
    return (
      <div className="px-6 py-8 max-w-4xl mx-auto">
        <div className="text-center">
          <div className="p-4 bg-blue-100 dark:bg-blue-900/30 rounded-lg max-w-md mx-auto">
            <p className="text-blue-600 dark:text-blue-400">
              Erreur lors du chargement du prêt
            </p>
            <button
              onClick={handleBack}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex justify-end gap-4 pt-6 border-t border-gray-200 dark:border-gray-700">
            ...
            <button
              type="submit"
              disabled={
                isSubmitting || isUpdating || isLoadingClients || isLoadingPret
              }
