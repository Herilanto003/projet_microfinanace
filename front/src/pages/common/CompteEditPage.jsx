...
const compteSchema = z.object({
  ...
  statut_compte: z.enum(["Active", "Inactive", "Suspendu"], {
...
export default function CompteEditPage() {
  ...
  const [updateCompte, { isLoading: isLoadingUpdateCompte }] =
    useUpdateCompteMutation();
  ...
  useEffect(() => {
    if (compteData?.data) {
      ...
      reset({
        ...
        description_compte: compte.description_compte || "",
        solde_compte: compte.solde_compte || "0.00",
        statut_compte: compte.statut_compte || "Active",
        client_id: compte.client_id?.toString() || "",
      });
    }
  }, [compteData, reset]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // Conversion du solde en number si nécessaire
      const compteData = {
        ...data,
      ...
      const response = await updateCompte({ id, ...compteData });
      ...
      <div className="bg-gray-50 dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex justify-end gap-4 pt-6 border-t border-gray-200 dark:border-gray-700">
            ...
            <button
              type="submit"
              disabled={isLoadingUpdateCompte}
